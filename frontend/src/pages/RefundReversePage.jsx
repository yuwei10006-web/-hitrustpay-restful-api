...
import { getMerchantConfig, refundReversePayment } from '../api/paymentApi';
import AppLayout from '../components/AppLayout';
import '../pages/Checkout.css';
import TransactionResultCard from '../components/TransactionResultCard';

export default function RefundReversePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!orderNumber.trim()) {
      return setError('請輸入要取消退款的訂單編號');
    }

    setLoading(true);
    try {
      const response = await refundReversePayment({ merid, orderNumber, queryFlag });
      if (!response.success) {
        setError(response.message || '取消退款失敗，請稍後再試');
        return;
      }
  ...
  return (
    <AppLayout title="取消交易退款" subtitle="取消已退款的訂單，此為整筆取消，不支援部分金額">
      <form className="checkout-card" onSubmit={handleSubmit}>
        ...
        <div className="section">
          ...
          <div className="grid-2">
            ...
            <div className="field">
              <label>訂單編號 *</label>
              <input value={orderNumber} onChange={(e) => setOrderNumber(e.target.value)} placeholder="要取消退款的原始訂單號" />
            </div>
            <div className="field">
