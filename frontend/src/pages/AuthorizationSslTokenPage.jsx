...
import { startAuthorizationSslToken } from '../api/paymentApi';
import AppLayout from '../components/AppLayout';
import './Checkout.css';
import TransactionResultCard from '../components/TransactionResultCard';

export default function AuthorizationSslTokenPage() {
  const form = useOrderForm({ defaultOrderDesc: '交易序號直接授權測試' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await startAuthorizationSslToken(payload);
      if (!response.success) {
  ...
  return (
    <AppLayout title="直接授權含交易序號(Token)" subtitle="用信用卡綁卡產生的交易序號扣款，不需再輸入卡號">
      <form className="checkout-card checkout-card--wide" onSubmit={handleSubmit}>
        ...
        <div className="section">
          ...
          <p className="field-hint">交易序號需先透過「信用卡綁卡交易授權」取得，此交易不支援 3DS 驗證</p>
        </div>
        ...
        {result && (
          <TransactionResultCard
            ...
            rows={[
              ...
              { label: '交易類別', key: 'type', value: 'TrxTokenAuthSSL' },
